...
class LocalNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        ...
        assert x.shape[-3] == self.color_channels*4, f"Expected {self.color_channels*4}-channel image, got {x.shape[-3]}" # total 4 images
        ...
        # Sum four 3-channel color images two make one color image
        x = torch.stack([(x[:,0,:,:]+x[:,3,:,:]+x[:,6,:,:]+x[:,9,:,:])/4, 
                       (x[:,1,:,:]+x[:,4,:,:]+x[:,7,:,:]+x[:,10,:,:])/4,
                       (x[:,2,:,:]+x[:,5,:,:]+x[:,8,:,:]+x[:,11,:,:])/4], dim=-3)
        return x + z
