...
class HyperLinearEmbedder(nn.Module):
    ...
    def __init__(self, n_params: int, n_layers: int, dim_w: torch.Size, dim_b: Union[torch.Size, None],
    ...
    def forward(self, h: torch.Tensor, index: torch.Tensor) -> Tuple[torch.Tensor, Union[torch.Tensor, None]]:
...
class HyperLinear(nn.Module):
    ...
    def __init__(self, target: Union[Linear, nn.Linear], embedder: nn.Module, index: torch.Tensor):
        super().__init__()
        self.target = target
    ...
    @classmethod
    def build(cls, n_params: int, targets: List[Union[Linear, nn.Linear]],
              mid_channels: int = 64, init_weights: Callable = lambda x: init.xavier_normal_(x, 2e-3)):
...
class HyperConv2d(nn.Module):
    ...
    def __init__(self, target: Union[Conv2d, nn.Conv2d], embedder: nn.Module, index: torch.Tensor):
        super().__init__()
        self.target = target
        self.embedder = embedder
        self.register_buffer("index", index)
    ...
    @classmethod
    def build(cls, n_params: int, targets: List[Union[Conv2d, nn.Conv2d]],
              mid_channels: int = 64, init_weights: Callable = lambda x: init.xavier_normal_(x, 2e-3)):
        """ Build a list of hyper-modules for each target modules.
