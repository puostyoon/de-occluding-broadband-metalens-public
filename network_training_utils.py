...
def find_latest_checkpoint(result_path):
    ...
    Args:
        result_path (str): The directory path where checkpoint files are saved (args.result_path).
    
    Returns:
        str or None: The full path to the latest checkpoint file. Returns None if no file is found.
    """
    
    if not os.path.isdir(result_path):
        print(f"Warning: the result directory could not be found: {result_path}") # Directory not found warning remains in Korean for user
        return None

    # Filter files ending with .pt, like 'training_state_001.pt'
    ...
    # Regular expression for extracting numbers (finds all consecutive digits)
    number_pattern = re.compile(r'(\d+)')

    print(f"Find the newest checkpoint in '{result_path}'...") # Searching for checkpoint message remains in Korean for user

    for filename in pt_files:
        # Find all numbers in the filename and convert them to a list of integers.
        numbers = [int(n) for n in number_pattern.findall(filename)]
        
        if numbers:
            # The last number in the filename is considered the 'epoch' number.
            current_epoch = numbers[-1]
        
            # Compare with the largest epoch number found so far.
            if current_epoch > best_match['epoch_number']:
                best_match['epoch_number'] = current_epoch
                best_match['filepath'] = os.path.join(result_path, filename)
            # If numbers are equal, the tie could be broken by string comparison,
            # but for simplicity, only the largest number is used as the criterion.

    if best_match['filepath']:
        print(f"Found the newest epoch ({best_match['epoch_number']}) checkpoints.") # Checkpoint found message remains in Korean for user
    else:
        print("Could not find any checkpoints.") # No checkpoint found message remains in Korean for user

    return best_match['filepath']
